...
export default class uploadView extends Component {
  state = {
    loading: false,
    staName:this.props.sourceType=='source_type_56'?'结清':'导入',
  };
  columns = [
    ...
    { title: '上传文件名', dataIndex: 'name' },
    {
      ...
      render: text => {
        return text == 'import_status_1' ? `待${this.state.staName}` : `${this.state.staName}完成`;
      },
    },
          ...
          {record.importStatusCode == 'import_status_1' && (
            <div>
              <a href="javascript:;" onClick={this.importOper(record)}>
                {record.sourceTypeCode=='source_type_56'?'结清':'导入'}
              </a>
              &nbsp;&nbsp;&nbsp;&nbsp;
      ...
      onOk() {
        ...
        httpCommonClient.post(`/warning/v1.0/warning/importRecord/import`, { id: record.id }).then(({ data = {} }) => {
          if (data.code === 200) {
            ...
            self.handleSearch();
            if (self.props.sourceType=='source_type_56') {
              Modal.success({
                title: '提示信息：',
