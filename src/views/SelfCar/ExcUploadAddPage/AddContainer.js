...
import { FETCH_RESERVED_LIST, UPDATE_RESERVED_QUERY } from 'redux/modules/selfcar/exUploadAdd';
...
export class excUploadAll extends Component {
  ...
  importOper = record => () => {
    ...
    confirm({
      ...
      onOk() {
        ...
        httpCommonClient.post(`/self-car/v1.0/excelUpload/importCars/inc/${record.batchno}`, {}).then(respone => {
  ...
  deleOper = record => () => {
    ...
    confirm({
      title: '请确认是否删除该文件?',
      onOk() {
        httpCommonClient.deleteMeth(`/self-car/v1.0/excelUpload/delete/inc/${record.fileId}`).then(respone => {
          if (respone.data.code === 200) {
  ...
  render() {
    ...
    let uploadConfig = {
      name: 'file',
      action: '/self-car/v1.0/excelUpload/upload/inc',
...
const mapStateToProps = store => ({
  ...
  list: store.exUploadAdd.reserved_add.list,
  paging: store.exUploadAdd.reserved_add.paging,
  query: store.exUploadAdd.reserved_add.query,
});
