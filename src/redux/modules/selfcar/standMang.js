...
export const fetchSoldTodo = createAction(FETCH_SOLD_TODO, (data, paging) => {
  let url = '/self-car/v1.0/standingBook/find/spot/list';
  return httpCommonClient.postWithPaging(url, { ...data, spottesttype: 20 }, paging);
});
export const fetchSoldGone = createAction(FETCH_SOLD_GONE, (data, paging) => {
  let url = '/self-car/v1.0/standingBook/find/spot/list';
  return httpCommonClient.postWithPaging(url, { ...data, spottesttype: 30 }, paging);
});
