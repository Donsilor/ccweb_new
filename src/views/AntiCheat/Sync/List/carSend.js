...
import { getCarsend, updateCarsend } from 'redux/modules/carSync';
import View from './listView';

const mapStateToProps = store => ({
    loading: store.carSync.loading,
    list: store.carSync.send.list,
    paging: store.carSync.send.paging,
    query: store.carSync.send.query,
    columns: columns,
    exportUrl: '/warning/v1.0/sync/show/export/car/send'
});

function mapDispatchToProps(dispatch) {
    return {
        fetch: (data, paging) => dispatch(getCarsend(data, paging)),
        updateQuery: data => {
            dispatch(updateCarsend({ ...data }));
        },
    };
}
