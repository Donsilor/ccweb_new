...
import { getJiefangOut, updateJiefangOut } from 'redux/modules/carMatching';
...
class thirdMatch extends Component {
  ...
  handleSearch = (formValues, page, pageSize) => {
    ...
    const { importDate, outboundDate } = formValues;
    if (importDate) {
    ...
    if (outboundDate) {
      const [startTime, endTime] = outboundDate;
      value.outboundStartDate = startTime && startTime.format('YYYY/MM/DD');
      value.outboundEndDate = endTime && endTime.format('YYYY/MM/DD');
      delete value.outboundDate;
    }
  ...
  handleExport = () => {
    ...
    const { importDate, outboundDate } = value;
    if (importDate) {
      const [startTime, endTime] = importDate;
      value.importStartDate = startTime && startTime.format('YYYY/MM/DD');
      value.importEndDate = endTime && endTime.format('YYYY/MM/DD');
    ...
    httpBufferClient
      .submit('/warning/v1.0/matching/outboundJiefang/export', value)
      .then(payload => {
            ...
            <a
              ...
              onClick={() => {
                ...
                confirm({
                  ...
                  onOk() {
                    httpCommonClient
                      .post(`/warning/v1.0/matching/outboundJiefang/noMatch`, { id: record.id })
                      .then(({ data = {} }) => {
                        if (data.code === 200) {
            ...
            <a
              ...
              onClick={() => {
                ...
                confirm({
                  ...
                  onOk() {
                    httpCommonClient
                      .post(`/warning/v1.0/matching/outboundJiefang/cancelMatched`, { id: record.id })
                      .then(({ data = {} }) => {
                        if (data.code === 200) {
                          message.success(data.message);
                          self.handlePageChange();
              ...
              onClick={() => {
                ...
                confirm({
                  ...
                  onOk() {
                    ...
                    httpCommonClient
                      .post('/warning/v1.0/matching/outboundJiefang/batchMatchBill', value)
                      .then(({ data = {} }) => {
                        if (data.code === 200) {
                          message.success(data.message);
                          self.handlePageChange();
              ...
              onClick={() => {
                ...
                confirm({
                  ...
                  onOk() {
                    ...
                    httpCommonClient
                      .post('/warning/v1.0/matching/outbound/syncOutbound', { billbrand: 12 })
                      .then(({ data = {} }) => {
                        if (data.code === 200) {
                          message.success(data.message);
                          self.handlePageChange();
...
const mapStateToProps = store => ({
  ...
  list: store.carMatching.jiefangOut.list,
  paging: store.carMatching.jiefangOut.paging,
  query: store.carMatching.jiefangOut.query,
});

function mapDispatchToProps(dispatch) {
  return {
    fetch: (data, paging) => dispatch(getJiefangOut(data, paging)),
    updateQuery: data => {
      dispatch(updateJiefangOut({ ...data }));
    },
  };
}
